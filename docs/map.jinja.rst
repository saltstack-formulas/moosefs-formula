...
After loading values from all sources, it will try to include the ``salt://{{ tplroot }}/libs/post-map.jinja`` Jinja file if it exists which can post-process the ``mapdata`` variable.
